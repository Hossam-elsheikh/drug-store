            ...
            <>
                    ...
                    {({ isSubmitting }) => (
                        <Form className="space-y-5">
                            ...
                            <div className="relative">
                                <CustomInput name='password' label={f("password")} placeholder={f('passHolder')} type={showPassword ? "text" : "password"} />
                ...
                <footer>
                    <div className='flex justify-center gap-2'>
                        ...
                        {variant === "full" && <Link href={type === 'sign-in' ? `/${locale}/sign-up` : `/${locale}/sign-in`} className="form-link font-semibold text-[#198AB0] hover:text-[#282A3F] ">
                            {type === 'sign-in' ? f('signUp') : f('signIn')}
                        </Link>}
                        {variant === 'drawer' && <FormButton type={type} setType={setType} />}
                    </div>
                    {variant === 'drawer' &&
                        <Link href={`/${locale}/${type}`} className='mt-10 flex gap-1 justify-center font-medium'>
                            {f('expand')} <Expand className='size-[18px] my-auto' />
                        </Link>
                    }
                </footer>
            </>
